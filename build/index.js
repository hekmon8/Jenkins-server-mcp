...
class JenkinsServer {
        ...
        this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
            tools: [
                ...
                },
                {
                    ...
                    description: 'List Jenkins jobs whose most recent build failed, sorted by most recent failure time.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            limit: {
                ...
                },
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            jobPath: {
                                type: 'string',
                                description: 'Path to the Jenkins job (e.g., "job/MyJob" or "view/xxx/job/MyJob")',
                            },
                        },
                        required: ['jobPath'],
                    },
    ...
    async listRecentFailedJobs(args) {
    ...
    async countFailedJobs() {
        ...
        const jobs = response.data.jobs || [];
        const failedCount = jobs.filter((job) => job.lastBuild?.result === 'FAILURE').length;
        return {
            content: [
                {
                    type: 'text',
    ...
    async getFailedBuildLog(args) {
        if (!args?.jobPath) {
            throw new McpError(ErrorCode.InvalidParams, 'jobPath is required for get_failed_build_log');
        }
        ...
        const logResponse = await this.axiosInstance.get(`/${args.jobPath}/${buildNumber}/consoleText`);
        return {
            content: [
                {
                    type: 'text',
    ...
    async createJenkinsUser(args) {
        const { username, password, fullName, email } = args || {};
        if (!username || !password) {
            throw new McpError(ErrorCode.InvalidParams, 'username and password are required to create a Jenkins user');
        }
        // Get CSRF crumb
        const crumbResp = await this.axiosInstance.get('/crumbIssuer/api/json');
        const crumbField = crumbResp.data.crumbRequestField;
